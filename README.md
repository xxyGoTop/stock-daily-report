...
每只自选会对照 `npm run pick` 的五套算法打标签：主策略、命中的算法（年新高 / 深调回升 / 火车轨 / 每日观察 / 五日线）、基底序号、涨幅榜第一版、所属板块 RPS5。没对上任何算法也会标「未命中」，方便对照。
...
- **自选评估**：传入的股票跳过成交额预筛，用同一套尾盘规则给出可买 / 观察 / 不买
...
输出分三块。**【自选尾盘评估】**（传了股票名/代码时）不走成交额预筛，ST、冷门也会给结论。**【尾盘可买】**是现价就能下手的市场扫描结果，每只给买入区间、止损止盈、建议仓位（跟着大盘信号浮动，大盘 avoid 时降到 0）、买入理由和风险提示——理由里只放支持做多的因子，利空一律进风险提示，两者不混。**【观察区】**是技术面合格但当前价位追不得的（多为乖离 >6.5%），标明回踩到哪个价位可以再评估。
...
src/
  ...
  crawl/
    eastmoney.js        行情 / K线 / 股票列表 / 板块快照（断联时降级）
    fallback.js         新浪/腾讯兜底：活跃列表、批量报价、涨跌家数、时钟
    decode.js           按响应头解码（腾讯/新浪 GBK）
    tencent.js          腾讯行情补名称 / 五档盘口
    cninfo.js           巨潮公告关键词检索
    hotNews.js          今日热点新闻
...
东方财富（行情、K 线、资金流、概念、热点新闻）、巨潮资讯（公告）、腾讯/新浪（名称联想、实时报价、列表兜底）、雪球（大 V 观点）。全部为公开接口，请合理控制请求频率。

### 东财断联时的降级

东财 push2 节点有时会整组不可用。关键取数会自动换源，控制台会打「降级…」一行：

| 能力 | 主源 | 断联后 |
|------|------|--------|
| 日 K 线 | 东财 / 腾讯 / 新浪（动态排序） | 已有三源轮换 |
| 活跃股票列表 | 东财 clist | 新浪行情中心分页 |
| 指数实时 | 东财 ulist | 腾讯批量 → 新浪 hq |
| 涨跌家数 / 强势股 | 东财指数字段 | 新浪全市场扫表近似 |
| 行情时钟 | 东财 stock/get | 腾讯 / 新浪上证时间 |

降级后没有主力净流入、行业分类可能为空，涨停池（专属接口）仍无法替代——对应模块会空着或跳过，不会拿假数据硬凑。报告「数据新鲜度」行会标出 `已降级：股票列表=sina` 之类。
